...
const en: BuyerSharesCopy = {
  ...
  h1: 'Buyer shares',
  leadHtml: `Every thread highlighted here comes from <strong>Reddit buyers who shop the Oopbuy Spreadsheet workflow</strong>—they pull product links from the spreadsheet-style catalogue and <strong>check out on Oopbuy</strong>, then share haul shots, QC talk, packaging photos, or <strong>international transit timing</strong> back in the community. <strong>Each story is one full-width card per row</strong> (not side-by-side grids), and within each section we list <strong>newer posts first</strong> by our editorial date stamp. General haul commentary is grouped first; timelines that emphasize <strong>transit windows</strong> sit in the blue section below. Official platform excerpts live under <a href="{newsUrl}">News &amp; guides</a>; workflow steps under <a href="{howToUrl}">How to buy</a>.`,
...
const de: BuyerSharesCopy = {
  ...
  h1: 'Käufer-Stimmen',
  leadHtml: `Die verlinkten Fäden stammen von <strong>Reddit-Nutzern, die den Oopbuy-Spreadsheet-Workflow nutzen</strong>: Links aus dem Spreadsheet-Katalog, Checkout auf <strong>Oopbuy</strong>, danach Fotos zur Qualität, Verpackung oder <strong>Versanddauer</strong>. Jeder Beitrag steht auf <strong>einer eigenen Breite/Zeile</strong>; neue Zusammenfassungen erscheinen <strong>oben nach Datum</strong>. Oberer Block = Haul-/QC-Gespräche; blauer Bereich darunter legt den Schwerpunkt auf <strong>Transit-Fenster</strong>. Offizielle Plattform-Auszüge: <a href="{newsUrl}">News &amp; Anleitungen</a>; Ablauf: <a href="{howToUrl}">So kaufen</a>.`,
...
const pt: BuyerSharesCopy = {
  ...
  leadHtml: `Os tópicos aqui são de <strong>compradores Reddit no fluxo Oopbuy Spreadsheet</strong>: descobrir links pelo catálogo estilo spreadsheet, <strong>pagar no Oopbuy</strong> e voltar ao Reddit com fotos do haul, QC, embalagem ou <strong>prazos de trânsito</strong>. <strong>Cada história usa um cartão inteiro por linha</strong>; entradas <strong>novas ficam primeiro</strong> segundo o nosso carimbo de data. Primeiro blocão = haul/QC geral; a faixa azul embaixo enfatiza <strong>linhas do tempo</strong>. Trechos oficiais em <a href="{newsUrl}">Notícias &amp; guias</a>; passo a passo em <a href="{howToUrl}">Como comprar</a>.`,
...
const es: BuyerSharesCopy = {
  ...
  leadHtml: `Los hilos enlazados vienen de <strong>usuarios Reddit en el flujo Oopbuy Spreadsheet</strong>: buscan enlaces del catálogo, <strong>compran en Oopbuy</strong> y comparten fotos del haul/QC/embalaje o <strong>franjas de tránsito</strong>. <strong>Cada relato va en una tarjeta a todo ancho</strong>; ordenamos las <strong>novedades primero</strong> por fecha. Arriba: comentarios de haul/QC general; zona azul debajo para <strong>líneas de tiempo</strong> de envío. Extractos de plataforma en <a href="{newsUrl}">Noticias &amp; guías</a>; cómo hacerlo en <a href="{howToUrl}">Cómo comprar</a>.`,
...
const it: BuyerSharesCopy = {
  ...
  leadHtml: `I thread puntati sono di <strong>acquirenti Reddit che usano il flusso Oopbuy Spreadsheet</strong>: link dal catalogo tipo foglio di calcolo, <strong>pagamento su Oopbuy</strong>, poi post con foto haul/QC/imballi o <strong>finestre di transito internazionale</strong>. <strong>Ogni scheda occupa una riga intera</strong>; le <strong>aggiunte più recenti vengono prima</strong> (data su questa pagina). In alto contenuti più generici sulla qualità/acquisti; la fascia blu sotto evidenzia le <strong>cronologie di spedizione</strong>. Estratti dal sito: <a href="{newsUrl}">Notizie &amp; guide</a>; tutorial: <a href="{howToUrl}">Come acquistare</a>.`,
...
const pl: BuyerSharesCopy = {
  ...
  h1: 'Głosy kupujących',
  leadHtml: `Wszystkie wątki to <strong>Reddit dla przepływu Oopbuy Spreadsheet</strong>: odkrycie linków w katalogu, <strong>płatność na Oopbuy</strong>, potem fotki haul/QC lub <strong>kalendarze przesyłek międzynarodowych</strong>. <strong>Każda relacja zajmuje szeroką kartę w osobnym rzędzie</strong>; <strong>najświeższe wpisy na początku</strong>. U góry ogólne uwagi jakościowe niżej niebiesko — <strong>okna tranzytu</strong>. Oficjalne fragmenty: <a href="{newsUrl}">Aktualności i poradniki</a>; kolejność kroków: <a href="{howToUrl}">Jak kupować</a>.`,
